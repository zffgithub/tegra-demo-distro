...
-------------------------
    sudo git clone https://github.com/zffgithub/tegra-demo-distro.git
    cd tegra-demo-distro
    sudo docker build -t build-yocto:1.0 docker/
    # Mac M1
    # sudo docker build -t build-yocto:1.0 -f docker/Dockerfile_arm docker/
...
Type the following command to instantiate a clean development environment for the Yocto Project:
    ...
    cd ~
    # 启动编译环境 8000端口可以提供默认web服务
    sudo docker run -ti -d -p 8080:8000 --volume=${PWD}/shared:/home/build/shared build-yocto:1.0
    # 启动exit后自动删除的编译环境
    # sudo docker run -ti --rm --volume=${PWD}/shared:/home/build/shared build-yocto:1.0
    # 测试离线编译
    # sudo docker run -ti --net=none --volume=${PWD}/shared:/home/build/shared build-yocto:1.0
    # sudo docker run -ti --net=none --rm --volume=${PWD}/shared:/home/build/shared build-yocto:1.0 bash -c 'bash -ex start.sh'

    # 正常编译
    sudo docker run --rm --volume=${PWD}/shared:/home/build/shared build-yocto:1.0 bash -c 'bash -ex start.sh'

#### 离线编译需要删掉 recipes-test
    sudo rm -rf ./layers/meta-tegra-community/recipes-test
